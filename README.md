...
## GitHub Pages vs README on github.com

- The page **`https://github.com/<owner>/<repo>`** is the **GitHub repository** view. The rendered **README** there is **not** your deployed website.
- After you enable **GitHub Actions** as the Pages source (below), the **live site** is served from a **`*.github.io`** URL, for example:
  - **Project site:** `https://<owner>.github.io/<repo>/` — you should see the **Next.js landing** (built `index.html`), not the README.
  - **User/org site** (repo named `<owner>.github.io`): `https://<owner>.github.io/`.
- **CI/CD:** push to `main` runs `.github/workflows/deploy-gh-pages.yml`, which builds a **static export** of `apps/web` and deploys the `apps/web/out` folder via `actions/upload-pages-artifact` + `actions/deploy-pages`.
...
- **Chosen approach (A):** `apps/web` uses `next build` with `NEXT_STATIC_EXPORT=1` → `output: 'export'` when you run `npm run build:web:static` or CI. This fits GitHub Pages (static files only).
- **Pros:** one Next app, marketing + app shell ship together; automated deploy from `main`.
- **Cons:** no SSR/ISR/Node server on Pages; dynamic dashboard URLs are not fully pre-rendered (we emit a single placeholder path per `[id]` route so the export build satisfies Next 15 — real IDs work only via client navigation after load, not as guaranteed static files). For a **marketing-only** slice, that is usually fine.
- **Alternative (B):** if static export ever becomes incompatible, add a small `apps/marketing-site` (or `marketing/`) that only contains public routes and export that instead; point the same workflow at its `out/` directory.

### GitHub Pages + API

**GitHub Pages does not run NestJS, PostgreSQL, Redis, or the worker** — only static HTML/JS/CSS.

1. **Landing / marketing only on Pages:** set `NEXT_PUBLIC_WEB_URL` (+ optional `NEXT_PUBLIC_BASE_PATH`). You do not need a public API for visitors who only read `/`, `/pricing`, `/legal/*`.
2. **Full product test from the internet:** deploy the API (and DB/Redis/worker) to a **VPS, Fly.io, Railway, Render, etc.** Set `NEXT_PUBLIC_API_URL` in Actions variables to that **https** API origin (not `http://localhost:4000`). CORS on the API must allow your Pages origin.

A minimal **self-hosted DB/Redis** reference is `docker-compose.prod.yml` (datastores only); run/build the API and worker separately.

### Local static build (GitHub Pages preview)

From the monorepo root (requires `npm install` so `cross-env` is available):

```bash
npm run build:web:static
```

Output directory: `apps/web/out` (must contain `index.html` at the root of the published site, or under `basePath` as generated by Next).

For a **project** site locally, mirror CI:

```bash
# PowerShell
$env:NEXT_STATIC_EXPORT="1"
$env:NEXT_PUBLIC_BASE_PATH="/my-repo"
$env:NEXT_PUBLIC_WEB_URL="https://<owner>.github.io/my-repo"
npm run build --workspace @email-saas/shared
npm run build --workspace @email-saas/web
```
